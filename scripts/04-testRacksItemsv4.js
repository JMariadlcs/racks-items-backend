/* This Script is used to deploy RacksItems.sol contract into a TESTNET
    ...
    or 'yarn hardhat run scripts/04-testRacksItemsv4 --network mumbai' */
...
async function test() {
    ...
    const racksItems = await ethers.getContract("RacksItemsv4", deployer)
    ...
    console.log("RacksItemsv4 address: " + racksItems.address.toString())
    ...
    const setCasePricetTx = await racksItems.setCasePrice('1000', { gasLimit: 9999999 })
    console.log("Changing case price to " + setCasePricetTx.toString() + "...")
    const racksTokenMintTxReceipt = await setCasePricetTx.wait(1)
    const getCasePriceTx = await racksItems.getPriceCase()
