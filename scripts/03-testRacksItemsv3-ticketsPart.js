/* This Script is used to deploy RacksItems.sol contract into a TESTNET
    ...
    or 'hh --network mumbai run scripts/03-testRacksItemsv3-ticketsPart.js' */
...
async function test() {
        ...
        const racksItems_user1 = await ethers.getContract("RacksItemsv3", user_1)
        const racksToken = await ethers.getContract("RacksToken", deployer)
        const racksToken_user1 = await ethers.getContract("RacksToken", user_1)
        ...
        */
        ...
        const listTicketTx = await racksItems.listTicket(2, 1, 5, { gasLimit: 9999999 })
        ...
        const approveRacksTokenTx4 = await racksToken_user1.approve(racksItems.address, 90000000000000000000n)
        const approveRacksTokenTxReceipt2 = await approveRacksTokenTx4.wait(1);
        console.log("RacksToken approved!")

        console.log("Buying ticket...")
        const buyTicketTx = await racksItems_user1.buyTicket(0, { gasLimit: 9999999 })
